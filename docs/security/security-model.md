...
See the [Networking requirements](../reference/system-requirements#networking-requirements) section of System requirements for the canonical inbound/outbound table, and [Data flow and privacy](./data-flow-and-privacy) for the inventory of what crosses each boundary.

## Attack surface

![nginx as the only TCP 443 listener, routing each path to an internal-only upstream.](../img/reverse-proxy-listeners.svg)
...
- **Centrally managed patching**. Linux host updates, Windows VM updates, driver updates, and CDS-version updates are delivered through the Hub. The Hub is the authoritative record of what was installed, when, and by whom. See [CID Hub architecture](./cid-hub-architecture) for the availability SLA, and [Traceability and compliance](./traceability-and-compliance) for the patch policy and rollback posture.
...
The full split (what Agilent owns versus what the customer owns across all CID surfaces) is enumerated in the [Shared responsibility](#shared-responsibility) section.
...
The procedure side of this flow lives in [Activate a CID](../howto/onboarding/activate-a-cid). The data exchanged during activation is enumerated in [Data flow and privacy](./data-flow-and-privacy).

### Certificate lifecycle and revocation

- **Managed lifecycle**. Device certificates are issued directly by AWS IoT Core during activation and carry AWS's default validity. The CID does not rely on customer-managed public key infrastructure (PKI) or manual certificate tracking.
...
|---|---|---|
| **Network firewall and segmentation** | None | Configuring the corporate firewall to permit the outbound domains listed in the [Internet requirements](../reference/system-requirements#internet-requirements) section of System requirements, and isolating the Instrument NIC's LAN/VLAN from the corporate WAN and the internet. |
| **Physical security and placement of the CID** | None | Securing the lab or facility where the CID resides, using the same physical access controls that protect the instruments it sits beside. Because it is a fanless, passively cooled appliance (see [Hardware and bundle](../reference/hardware-and-bundle)), give it room to dissipate heat: keep its vents clear, do not stack units or place them under other equipment, and keep it within its rated operating temperature. |
| **CID device identity** | X.509 client certificate provisioned at activation, rotated on the Agilent-managed cadence, and revocable through the Hub. | None |
...
| **CID Hub (SaaS) infrastructure** | AWS infrastructure, infrastructure patching, TLS termination, infrastructure monitoring, encryption in transit and at rest on the Hub side, and multi-tenant isolation. | None |
...
- [CID Hub architecture](./cid-hub-architecture): Hub-side AWS service inventory, tenant isolation, region and data-residency posture, and the availability SLA.
- [Data flow and privacy](./data-flow-and-privacy): the nine-category inventory of what crosses the CID ⇄ Hub boundary, plus PHI/PII stance and retention.
...
- [Traceability and compliance](./traceability-and-compliance): Activity Log model, retention, tamper protection, and the CID's relationship to 21 CFR Part 11 and EU GMP Annex 11.
