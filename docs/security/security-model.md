...
- **No customer-managed anti-malware product.** Agilent ships ClamAV on the CID. Customers cannot install a third-party agent (for example, CrowdStrike, Defender ATP, or SentinelOne) on either the Linux host or the embedded Windows VM. Endpoint-management products run on the CDS client machines, which the customer continues to own.
