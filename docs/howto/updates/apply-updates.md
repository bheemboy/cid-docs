...
- **Allow Changes** must be on for the CID. (Bulk updates skip any CID with **Allow Changes** off.)
...
- **Status:** **Ready**. The CID is online, the OpenLab Instrument Service is running, and OpenLab CDS is not in use. Applying updates while the status is **In Use** is allowed, but acquisitions and ongoing analyses on that CID may be impacted.
- **Updates:** **Ready**. All selected changes are downloaded. **Downloading** means at least one component is still being fetched and the install button is not yet active.
...
2. Select the checkbox for each CID you want to update. Use the **Status** and **Updates** column filters to narrow the list to candidates that are **Ready** in both columns.

3. Click **Apply Updates** in the toolbar above the table.
...
1. On the **CIDs** list, click the CID name to open its detail page.
...
1. Open the CID's detail page and select the **Software** tab.

2. Find the row for the component you want to install. The **Status** column shows where the component is in its lifecycle (for example, **Downloading**, **Ready to install**, **Installing**).

3. Click **Install** on that row.
...
If something goes wrong during installation, the CID is designed to stay usable. Components that already installed successfully in the same run keep working, and the **Recent activity** section of the CID's **Summary** page plus the [activity log](../monitoring/view-activity-logs) record both the failure and any recovery action.

How the failed component itself is handled depends on what it is:

- **OpenLab CDS, Linux update, and CID Connectivity** are protected by a two-slot update mechanism. If the new version cannot start, the CID automatically restores the previous version from the inactive slot and continues running.
- **Windows updates** are retried up to 5 times at 60-second intervals inside the Windows VM. If all retries fail, the update is marked failed and the CID keeps running on its current Windows patch level.
- **Drivers and add-ons** that fail to install leave the existing driver in place. The failed install is reported on the **Software** tab and in the Activity Log.
...
3. Reapply the update using any of the three methods above. You only need to retry the components that failed.

:::note
In the rare case where a CDS slot restore cannot complete (for example, the previous VM file is no longer usable), the Activity Log records this clearly. Recover the CID by running **Reset OpenLab CDS** from the [CID administration](../operations/cid-administration) page, which installs a fresh CDS VM from the slot baseline. Contact Agilent support if the reset does not return the CID to **Ready**.
:::

## See also

- [View CIDs](../monitoring/view-cids): check **Status**, **Updates**, and **Allow Updates** before starting a bulk update.
