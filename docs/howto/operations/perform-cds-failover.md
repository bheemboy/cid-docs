...
- You need to know which CID controls the instrument. If you do not, see [Identify the CID](#identify-the-cid).
...
- A printed **Instrument Controllers Report** or **Instrument Report** from Control Panel, kept near the bench. Reprint it whenever instruments are added, moved, or renamed so the copy on the bench stays current.
...
2. Open the **CIDs** list and select the CID you identified.
3. In the sidebar, click **Administration**.
...
2. Open the **CIDs** list and select the CID. In the sidebar, click **Administration**.
3. Click **Launch CDS Desktop**.
4. Select how to reach the CID:
...
Since CID Hub is unreachable, the [Retrieve the CDS Desktop password](#retrieve-the-cds-desktop-password) procedure will not work either. You will need the current password through some other route, such as a phone on cellular data or any other device that still has internet access.
...
![Browser tab showing the Windows VM console sign-in screen for a CID, with the CDS Desktop user](../../img/windows-vm-signin.png)

## Sign in and start CDS in failover

1. Sign in with the **CDS Desktop user** credentials you copied from the Administration tab.
2. Launch **Control Panel** from the Windows desktop. Because the OpenLab Server is unreachable, Control Panel prompts you to switch to Failover mode. Accept the prompt.
