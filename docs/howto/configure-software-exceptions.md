...
However, you may need one or more specific CIDs to use unique setups. For example, a CID might need a MicroGC driver, another might need a Headspace driver. And you would want to do this without adding those drivers to all CIDs.

To set up unique CIDs, as a first step must configure the specific CIDs to stop inheriting from the server. This allows you to manage their software components individually.
