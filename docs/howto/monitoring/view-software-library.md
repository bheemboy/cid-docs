...
- You must be signed in to CID Hub. Any signed-in user can view the **Software Library**.
- *(Optional, for the download procedure below)* You need write access to the CID Network Share configured on the OpenLab Server.
...
2. Use the menu on the left to switch between categories. Use the search field at the top-right of the content pane to filter the listed entries.
...
- **Actions:** Click the download icon to reveal the KVM image file (`.qcow2`) and its checksum file (`.sha256`). See [Download a CDS KVM image to a network share](#download-a-cds-kvm-image-to-a-network-share) below.
...
![Software Library Linux Update list with several published `YYYY.MM.DD` versions, the **Release Notes** link, and the **Release Date** column visible](../../img/software-library-linux-update.png)
...
- **OpenLab Sample Scheduler**, an OpenLab CDS add-on that gives lab staff a simple interface for managing their workload and sending samples to any instrument for analysis, and integrates with LIMS to automate sample assignment and result entry.
- **GPC** (Agilent GPC/SEC Software for OpenLab CDS), an OpenLab CDS add-on that adds Gel Permeation Chromatography and Size Exclusion Chromatography calibrations and molecular-weight calculations to data acquired through OpenLab CDS.
